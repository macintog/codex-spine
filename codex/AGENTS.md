...
- Use `codex/TOOLING.md` when the task needs the shared continuity packet, closeout flow, direct memory retrieval, or indexed code navigation.
- When prior wording matters or broad file scanning would otherwise be needed, use the memory and `jcodemunch` lanes in `codex/TOOLING.md` instead of guessing from recap or exploring filesystem-first.
