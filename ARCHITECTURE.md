...
This subsystem is the default public core. It is built around [@tobi/qmd](https://github.com/tobi/qmd) and exists to give Codex better startup context and retrieval without you having to dig through transcript files by hand. The advertised historical surface is deliberately small: `recent_session` handles only explicit topicless last-conversation questions, while `query` requires one exact project root, accepts one to three typed `lex`, `vec`, or `hyde` searches within it, and is followed by bounded `get` or `multi_get` evidence. A scoped miss fails closed. The older three search names remain hidden compatibility aliases for cached clients.

Questions about this repo or Git as it is now are answered from the current checkout and refs. Questions about a named past project, decision, or discussion go to memory before a hunt through some other tree. Exact names use lexical search first, then one source read. Semantic search is the one fallback if that misses. Do not reopen old filesystem paths just to double-check a hit. Index time is separate from how long the model then spends writing.
