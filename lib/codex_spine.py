...
def ensure_homebrew(*, non_interactive: bool) -> Path:
    ...
    if not prompt_yes_no(
        "First we'll install Homebrew, then install the Homebrew packages codex-spine uses: python, ripgrep, node, pnpm, uv, and jq. Continue?",
        default=True,
...
def prepare_generated_config_target(path: Path, *, non_interactive: bool) -> Path | None:
    if not path.exists():
        return None
    current = path.read_text(encoding="utf-8")
    ...
    if non_interactive or not sys.stdin.isatty():
        raise RuntimeError(
            f"refusing to overwrite unmanaged config in non-interactive mode: {path}. Move it aside or merge the settings you want to keep into codex/config/90-local.toml, then rerun `make install`."
        )
    if not prompt_yes_no(
        (
            f"Found an existing Codex config at {path}. "
            "Back it up to a timestamped .bak copy and replace it with the codex-spine managed config now?"
        ),
        default=True,
        non_interactive=non_interactive,
    ):
        raise RuntimeError(
            f"refusing to overwrite unmanaged config: {path}. Move it aside or merge the settings you want to keep into codex/config/90-local.toml, then rerun `make install`."
        )
    return backup_existing(path)
