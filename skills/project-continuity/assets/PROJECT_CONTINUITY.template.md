...
| Prior focus and advisory next-step candidate | `<declared checkpoint resolver, or tracked handoff when no resolver is declared>` | Never selects the task; reconcile its full subject and state anchor against the latest user request and primary evidence before use. An adopted resolver failure never falls back to tracked or recovery prose. |
